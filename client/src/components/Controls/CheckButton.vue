  ...
  <div class='hoverdown left flush action'>
    ...
    <div class="hoverdown--target -ml-14">
      ...
      <div class="border-double border-t-4 border-gray-600 flex flex-wrap">
        <div :class="css.btn()" @click="reset">
          Select All {{ is_staff && 'Required'}}
        </div>
...
export default {
  computed: {
    ...
    columns() {
      ...
      return range(Math.ceil(this.options.length / per_column))
        .map((i) => options.slice(i * per_column, (i + 1) * per_column))
    },
    options() {
  ...
  methods: {
    ...
    saveConstraints() {
      this.$todo("const saveConstraints = (required_constraints = constraints) => actions.savePuzzle({ required_constraints })")
    },
    reset() {

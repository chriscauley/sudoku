...
export default class Hoverdown extends React.Component {
  ...
  getClass = () => classNames('hoverdown', this.props.className, this.state.className)
  render() {
    ...
    let content_class = ''
    return (
      <div className={this.getClass()} onMouseOver={this.mouseover} onMouseOut={this.mouseout}>
        {this.props.children}
        <div className="hoverdown--target">
          <div className="hoverdown--links">
            <a className={css.icon('thumb-tack cursor-pointer', {active: pinned})} onClick={this.pin}/>
            <a className={css.icon('close cursor-pointer')} onClick={this.close}/>
          </div>
